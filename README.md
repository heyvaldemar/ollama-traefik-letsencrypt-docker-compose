...
[![OpenSSF Best Practices](https://www.bestpractices.dev/projects/14854/badge)](https://www.bestpractices.dev/projects/14854)
...
- **Ollama** latest stable (0.34.3) with automatic model installation from a configurable list.
...
Every release from v1.5.7 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `ollama-traefik-letsencrypt-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `ollama-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `ollama-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob ollama-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --bundle ollama-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/ollama-traefik-letsencrypt-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact ollama-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --provenance-path ollama-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/ollama-traefik-letsencrypt-docker-compose
```
